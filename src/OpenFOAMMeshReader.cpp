...
std::vector<int>
OpenFOAMMeshReader::parseIntListFromString(const std::string& line)
{
...
void OpenFOAMMeshReader::readPointsBinary(std::ifstream& file, int nPoints)
{
    ...
    while (file.get(c))
    {
        ...
        else if (!std::isspace(c))
        {
            throw std::runtime_error(
                "Expected '(' before binary points data, got: '" +
                std::string(1, c) + "'");
        }
    }
    ...
    for (int i = 0; i < nPoints; ++i)
    {
        ...
        {
            throw std::runtime_error(
                "Error reading binary point data at index " +
                std::to_string(i));
        }
        points.push_back(p);
    }
    ...
    while (file.get(c))
    {
        ...
        {
            throw std::runtime_error(
                "Expected ')' after binary points data, got: '" +
                std::string(1, c) + "'");
        }
    }
}
...
void OpenFOAMMeshReader::readFacesBinary(std::ifstream& file, int nFaces)
{
    ...
    while (file.get(c))
    {
        ...
        {
            throw std::runtime_error(
                "Expected '(' before binary faces indices, got: '" +
                std::string(1, c) + "'");
        }
    }
    ...
    for (int i = 0; i < nFaces; ++i)
    {
        ...
        {
            throw std::runtime_error(
                "Error reading binary face index at position " +
                std::to_string(i));
        }
    }
    ...
    while (file.get(c))
    {
        ...
        {
            throw std::runtime_error(
                "Expected ')' after binary faces indices, got: '" +
                std::string(1, c) + "'");
        }
    }
    ...
    {
        throw std::runtime_error(
            "Error parsing total face data count from line: '" + line + "' - " +
            e.what());
    }
    ...
    while (file.get(c))
    {
        ...
        {
            throw std::runtime_error(
                "Expected '(' before binary faces data, got: '" +
                std::string(1, c) + "'");
        }
    }
    ...
    for (int i = 0; i < totalData; ++i)
    {
        ...
        {
            throw std::runtime_error(
                "Error reading binary face data at position " +
                std::to_string(i));
        }
    }
    ...
    while (file.get(c))
    {
        ...
        {
            throw std::runtime_error(
                "Expected ')' after binary faces data, got: '" +
                std::string(1, c) + "'");
        }
    }
...
void OpenFOAMMeshReader::readOwnerBinary(std::ifstream& file, int nOwner)
{
    ...
    while (file.get(c))
    {
        ...
        {
            throw std::runtime_error(
                "Expected '(' before binary owner data, got: '" +
                std::string(1, c) + "'");
        }
    }
    ...
    for (int i = 0; i < nOwner; ++i)
    {
        ...
        {
            throw std::runtime_error(
                "Error reading binary owner data at index " +
                std::to_string(i));
        }
    }
    ...
    while (file.get(c))
    {
        ...
        {
            throw std::runtime_error(
                "Expected ')' after binary owner data, got: '" +
                std::string(1, c) + "'");
        }
    }
}

void OpenFOAMMeshReader::readNeighbourBinary(std::ifstream& file,
                                             int nNeighbour)
{
    // For labelList binary format: no list size prefix
    // Skip whitespace and read opening parenthesis
    ...
    while (file.get(c))
    {
        ...
        {
            throw std::runtime_error(
                "Expected '(' before binary neighbour data, got: '" +
                std::string(1, c) + "'");
        }
    }
    ...
    for (int i = 0; i < nNeighbour; ++i)
    {
        ...
        {
            throw std::runtime_error(
                "Error reading binary neighbour data at index " +
                std::to_string(i));
        }
    }
    ...
    while (file.get(c))
    {
        ...
        {
            throw std::runtime_error(
                "Expected ')' after binary neighbour data, got: '" +
                std::string(1, c) + "'");
        }
    }
}
...
void OpenFOAMMeshReader::readPoints()
{
    ...
    if (!file.is_open())
    {
        throw std::runtime_error("Cannot open points file: " + meshPath +
                                 "/points");
    }
    ...
    else
    {
        ...
        {
            throw std::runtime_error(
                "Error parsing number of points from line: '" + line + "' - " +
                e.what());
        }
...
void OpenFOAMMeshReader::readFaces()
{
    ...
    if (!file.is_open())
    {
        throw std::runtime_error("Cannot open faces file: " + meshPath +
                                 "/faces");
    }
    ...
    {
        throw std::runtime_error("Error parsing number of faces from line: '" +
                                 line + "' - " + e.what());
    }
...
void OpenFOAMMeshReader::readOwner()
{
    ...
    if (!file.is_open())
    {
        throw std::runtime_error("Cannot open owner file: " + meshPath +
                                 "/owner");
    }
    ...
    {
        throw std::runtime_error(
            "Error parsing number of faces from owner file, line: '" + line +
            "' - " + e.what());
    }
    ...
    std::cout << "Read owner information for " << owner.size() << " faces"
              << std::endl;
}
...
void OpenFOAMMeshReader::readNeighbour()
{
    ...
    {
        std::cout
            << "Note: neighbour file not found (may be empty for some meshes)"
            << std::endl;
        return;
    }
    ...
    {
        throw std::runtime_error("Error parsing number of internal faces from "
                                 "neighbour file, line: '" +
                                 line + "' - " + e.what());
    }
    ...
    std::cout << "Read neighbour information for " << neighbour.size()
...
void OpenFOAMMeshReader::readBoundary()
{
    ...
    if (!file.is_open())
    {
        throw std::runtime_error("Cannot open boundary file: " + meshPath +
                                 "/boundary");
    }
    ...
    {
        throw std::runtime_error(
            "Error parsing number of patches from boundary file, line: '" +
            line + "' - " + e.what());
    }
    ...
    for (int i = 0; i < nPatches; ++i)
    {
        ...
        line = getNextNonEmptyLine(file);
        patch.name = line;
        patch.name.erase(std::remove(patch.name.begin(), patch.name.end(), ' '),
                         patch.name.end());
        patch.name.erase(
            std::remove(patch.name.begin(), patch.name.end(), '\t'),
            patch.name.end());
        ...
        while (std::getline(file, line))
        {
            ...
            else if (line.find("nFaces") != std::string::npos)
            {
                ...
                nFacesStr.erase(
                    std::remove(nFacesStr.begin(), nFacesStr.end(), ' '),
                    nFacesStr.end());
                nFacesStr.erase(
                    std::remove(nFacesStr.begin(), nFacesStr.end(), ';'),
                    nFacesStr.end());
            ...
            else if (line.find("startFace") != std::string::npos)
            {
                ...
                startFaceStr.erase(
                    std::remove(startFaceStr.begin(), startFaceStr.end(), ' '),
                    startFaceStr.end());
                startFaceStr.erase(
                    std::remove(startFaceStr.begin(), startFaceStr.end(), ';'),
                    startFaceStr.end());
        ...
        std::cout << "  Boundary patch: " << patch.name
                  << " (type: " << patch.type << ", faces: " << patch.nFaces
                  << ", start: " << patch.startFace << ")" << std::endl;
    }
}
...
void OpenFOAMMeshReader::readCellZones()
{
    ...
    {
        std::cout << "Could not parse number of zones, assuming 0 zones"
                  << std::endl;
        return;
    }
    ...
    for (int i = 0; i < nZones; ++i)
    {
        ...
        line = getNextNonEmptyLine(file);
        zone.name = line;
        zone.name.erase(std::remove(zone.name.begin(), zone.name.end(), ' '),
                        zone.name.end());
        zone.name.erase(std::remove(zone.name.begin(), zone.name.end(), '\t'),
                        zone.name.end());
        ...
        while (std::getline(file, line))
        {
            ...
            if (line.find("cellLabels") != std::string::npos)
            {
                ...
                if (parenPos != std::string::npos)
                {
                    ...
                    cellData.erase(
                        std::remove(cellData.begin(), cellData.end(), ')'),
                        cellData.end());
                    cellData.erase(
                        std::remove(cellData.begin(), cellData.end(), ';'),
                        cellData.end());
                ...
                else
                {
                    ...
                    {
                        throw std::runtime_error(
                            "Error parsing number of cells in zone, line: '" +
                            line + "' - " + e.what());
                    }
        ...
        std::cout << "  Cell zone: " << zone.name << " ("
                  << zone.cellIndices.size() << " cells)" << std::endl;
    }
}
...
void OpenFOAMMeshReader::constructCells()
{
    ...
    for (auto& cell : cells)
    {
        ...
        if (nFaces == 4 && nTri == 4 && nQuad == 0)
            cell.type = "tet";
        else if (nFaces == 6 && nQuad == 6 && nTri == 0)
            cell.type = "hex";
        else if (nFaces == 5 && nQuad == 1 && nTri == 4)
            cell.type = "pyr";
        else if (nFaces == 5 && nQuad == 3 && nTri == 2)
            cell.type = "wedge";
        else
            cell.type = "unknown";
    }
