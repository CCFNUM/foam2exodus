    ...
    : casePaths(casePaths)
{
    if (casePaths.empty())
    {
        throw std::runtime_error(
            "No case paths provided for merged mesh reader");
    }
}

void MergedMeshReader::readMeshes()
{
    std::cout << "Reading " << casePaths.size()
              << " OpenFOAM meshes for merging..." << std::endl;

    // Read all meshes
    for (size_t i = 0; i < casePaths.size(); ++i)
    {
        std::cout << "  [" << (i + 1) << "/" << casePaths.size()
                  << "] Reading: " << casePaths[i] << std::endl;
        ...
        std::cout << "    Points: " << reader->getNumPoints()
                  ...
                  << ", Patches: " << reader->getNumBoundaryPatches()
                  << std::endl;
    ...
    std::cout << "  Total Boundary Patches: " << mergedBoundaryPatches.size()
              << std::endl;
...
void MergedMeshReader::mergeBoundaryPatches()
{
    ...
    for (const auto& [name, meshes] : originalNameToMeshes)
    {
        if (meshes.size() > 1)
        {
            ...
            {
                std::cout
                    << "\nDetected duplicate boundary names across meshes:"
                    << std::endl;
                hasDuplicates = true;
            }
    ...
    if (hasDuplicates)
    {
        std::cout << "  → Only duplicate names will be prefixed with mesh name"
                  << std::endl;
        std::cout << std::endl;
    }
...
void MergedMeshReader::mergeCellZones()
{
    ...
    for (const auto& [name, meshes] : originalZoneNameToMeshes)
    {
        if (meshes.size() > 1)
        {
            ...
            {
                std::cout
                    << "\nDetected duplicate cell zone names across meshes:"
                    << std::endl;
                hasDuplicates = true;
            }
            std::cout << "  '" << name << "' found in: ";
    ...
    if (hasDuplicates)
    {
        std::cout << "  → All zones prefixed with mesh name to ensure unique "
                     "element blocks"
                  << std::endl;
        std::cout << std::endl;
    }
    ...
    for (size_t meshIdx = 0; meshIdx < readers.size(); ++meshIdx)
    {
        ...
        for (const auto& zone : zones)
        {
            ...
            // The ExodusWriter creates element blocks based on zone names and
            // element types We need to ensure each mesh's zones produce
            // distinct element blocks
